...
class NotificationController extends GetxController {
  ...
  fetchNotification() async {
    ...
    try {
      ...
      var request =
          http.Request('GET', Uri.parse(ApiConstants.notificationUrl));
  ...
  markAsRead({Function(String?)? callBack, String? notificationId}) async {
    ...
    try {
      ...
      var request = http.Request('PATCH',
          Uri.parse(ApiConstants.markAsReadNotificationUrl(notificationId)));
      ...
      if (response.statusCode == 200) {
        print(decodedBody['message']);
        Get.toNamed(Routes.REQUEST_TO_PLAY, arguments: {
          'tournamentId': '${decodedBody['data']['attributes']['tournamentId']}'
        });
      } else {
        print('Error: ${response.statusCode}');
        Get.snackbar('Failed', decodedBody['message']);
